...
class PersonalizedServer(Server):
    ...
    def evaluate_round(
        ...
        for client_proxy, baseline_res, personalized_res in results:
            ...
            delta_loss = abs(personalized_res.loss - baseline_res.loss)
            ...
            for k in baseline_res.metrics.keys():
                ...
                if isinstance(baseline_val, float) and isinstance(personalized_val, float):
                    delta_metrics_dict[k] = abs(personalized_val - baseline_val)
        ...
        accuracy_x_axis = [delta_metric.metrics.get('balanced_acc', 0) for client_proxy, delta_metric in delta_metrics]
